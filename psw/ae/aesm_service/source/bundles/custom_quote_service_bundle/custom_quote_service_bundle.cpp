...
class CustomQuoteServiceImp : public IQuoteProviderService
{
private:

    bool initialized;
public:
    CustomQuoteServiceImp():initialized(false) {}

    ae_error_t start()
    {
        AESMLogicLock lock(custom_quote_mutex);
        if (initialized == true)
        {
            AESM_DBG_INFO("custom bundle has been started");
            return AE_SUCCESS;
        }

        AESM_DBG_INFO("Starting custom bundle");
        auto context = cppmicroservices::GetBundleContext();

        initialized = true;
        AESM_DBG_INFO("custom bundle started");
        return AE_SUCCESS;
    }
    void stop()
    {
        initialized = false;
        AESM_DBG_INFO("custom bundle stopped");
    }
    ...
    {
        AESM_LOG_WARN("get_att_key_id_num");
...
class Activator : public BundleActivator
{
  ...
  {
    auto service = std::make_shared<CustomQuoteServiceImp>();
...
#ifdef NO_CMAKE
CPPMICROSERVICES_INITIALIZE_BUNDLE(custom_quote_service_bundle_name)
#endif
